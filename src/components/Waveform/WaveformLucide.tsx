...
// Create a local atom for the current playing audio ID
const localPlayingAudioIdAtom = atom(
  (get) => get(currPlayingAudioIdAtom),
  (get, set, newValue: string | undefined) => {
    set(currPlayingAudioIdAtom, newValue)
  }
)

const WaveformLucide: FC<Props> = ({ filename }) => {
  const [currPlayingAudioId, setCurrPlayingAudioId] = useAtom(
    localPlayingAudioIdAtom
  )
